...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List likelihood_function2_exact(arma::vec y_arma,
                                NumericMatrix treetable_temp,NumericMatrix obs_to_nodes_temp,double a,double mu,double nu,double lambda,
  ...
  const Eigen::Map<Eigen::VectorXd> yy(y_arma.memptr(),
                                       y_arma.n_elem,
                                       1);
  ...
  //Note: using in-sample predictions, therefore
  //not using W_tilde
  //(only compute test data predictions for finally outputted models)
  //arma::vec whw_vars = (arma::sum((W_tilde*invHmat).t() % W_tilde.t(), 0)).t();
  arma::vec whw_vars = (arma::sum((Wmat*invHmat).t() % Wmat.t(), 0)).t();

  //arma::vec wo_means  = W_tilde*mapcoeffs;
  arma::vec wo_means  = Wmat*mapcoeffs;
  ...
  List ret(2);
  ret[0]=templik0; //double
  ret[1]=wrap(predprobstemp); //arma vec; (wrapped to NumericVector)
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List sumtree_likelihood_function2_exact(arma::vec y_arma,List sum_treetable ,List sum_obs_to_nodes,int n,double a,double nu,double lambda,
                                        double root_alg_precision,
                                        int maxit,
                                        double eps_f,
                                        double eps_g){
...
List get_best_split_exact(double less_greedy, double spike_tree, int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
                          arma::vec y_arma, //NumericVector resids,
                          arma::mat& data,NumericMatrix treetable,NumericMatrix tree_mat,
                          double a,double mu,double nu,double lambda,double c,double lowest_BIC,int parent,
  ...
  for(int l=0;l<terminal_nodes.size();l++){
    ...
    for(int k=0;k<w;k++){
      ...
      lik_listoutput=likelihood_function2_exact(y_arma,
                                                proposal_tree[0],proposal_tree[1],a,mu,nu,lambda,
                                                root_alg_precision,
                                                maxit,
                                                eps_f,
                                                eps_g);
...
List get_best_split_2_exact(double less_greedy, double spike_tree, int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
  ...
  for(int l=0;l<terminal_nodes.size();l++){
    ...
    for(int k=0;k<w;k++){
      ...
      lik_listoutput=likelihood_function2_exact(y_arma,
                                                proposal_tree[0],
                                                proposal_tree[1],a,mu,nu,lambda,
                                                root_alg_precision,
                                                maxit,
                                                eps_f,
                                                eps_g);
...
List get_best_split_sum_exact(double less_greedy, double spike_tree, int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
                              ...
                              double a,double mu,double nu,double lambda,double c,double lowest_BIC,
                              int parent,NumericMatrix cp_mat,double alpha,double beta,int maxOWsize,//int first_round,
                              List sum_trees,List sum_trees_mat,
                              arma::vec y_arma,
                              IntegerVector parent2,int i,
                              unsigned int min_num_obs_for_split,unsigned int min_num_obs_after_split,
                              double root_alg_precision,
                              int maxit,
                              double eps_f,
  ...
  for(int l=0;l<terminal_nodes.size();l++){
    ...
    for(int k=0;k<w;k++){
      ...
      if(is<List>(s)){
        ...
        lik_list=sumtree_likelihood_function2_exact(y_arma,
                                                    sum_trees2,sum_trees_mat2,y_arma.n_elem,a,nu,lambda,
                                                    root_alg_precision,
                                                    maxit,
                                                    eps_f,
                                                    eps_g);
        ...
        lik_list=sumtree_likelihood_function2_exact(y_arma,
                                                    st,st_mat,y_arma.n_elem,a,nu,lambda,
                                                    root_alg_precision,
                                                    maxit,
                                                    eps_f,
                                                    eps_g);
...
List get_best_split_sum_2_exact(double less_greedy, double spike_tree, int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
                                ...
                                int parent,List cp_matlist,double alpha,double beta,int maxOWsize,//int first_round,
                                List sum_trees,List sum_trees_mat,
                                arma::vec y_arma,//NumericVector y_scaled,
                                IntegerVector parent2,int i,
                                unsigned int min_num_obs_for_split,unsigned int min_num_obs_after_split,
                                double root_alg_precision,
                                int maxit,
  ...
  for(int l=0;l<terminal_nodes.size();l++){
    ...
    for(int k=0;k<w;k++){
      ...
      if(is<List>(s)){
        ...
        lik_list=sumtree_likelihood_function2_exact(y_arma,//y_scaled,
                                                    sum_trees2,sum_trees_mat2,y_arma.n_elem,a,nu,lambda,
                                                    root_alg_precision,
                                                    maxit,
                                                    eps_f,
...
double SS(arma::vec x, arma::vec y, double split,
  ...
  double tSS=arma::dot(p_1_minus_p%(y-meanAll),(y-meanAll));
...
List gridCP(arma::vec x,
  ...
  for(int i=1;i<(gridSize-1);i++) {
    ...
    {
      out[i-1]=xGrid;
      double testSS=SS(x,y,xGrid, p_1_minus_p);
...
arma::field<arma::vec> gridCP_arma(arma::vec x,
                                   arma::vec y,
                                   arma::vec p_1_minus_p,
                                   int gridSize = 10) {
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List make_gridpoint_cpmat(NumericMatrix data,
                          NumericVector resp,
                          int gridsize,
                          int num_cp,
                          arma::vec p_1_minus_p){
...
List make_gridpoint_cpmat_arma(arma::mat data,arma::vec resp,int gridsize,int num_cp,
  ...
  for(int i=0;i<numvar;i++){
    ...
    //arma::vec t=Rcpp::as<arma::vec>(resp);
    arma::field<arma::vec> ans1 = gridCP_arma(coli_ar,resp,p_1_minus_p,gridsize);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List make_pelt_cpmat(NumericMatrix data,
                     NumericVector resp,
                     double pen,
                     int num_cp){
  int err1=0;
...
List get_best_trees_exact(double less_greedy, double spike_tree, int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
  ...
  if(zero_split==1){
    ...
    List lik_templist=likelihood_function2_exact(y_arma,
                                                 tree_table[0],tree_mat[0],a,mu,nu,lambda,
                                                 root_alg_precision,
                                                 maxit,
                                                 eps_f,
                                                 eps_g);
  ...
  for(int j=0;j<num_splits;j++){
    ...
    for(int i=0;i<tree_table.size();i++){
      ...
      if(split_rule_node==1){
        if(j==0){
          best_subset=get_best_split_exact(less_greedy,spike_tree, s_t_hyperprior, p_s_t, a_s_t,b_s_t,num_obs,num_vars,lambda_poisson,
                                           y_arma,//resids(_,0),
                                           D1,tree_table[i],tree_mat[i],a,mu,nu,lambda,log(c),
                                           lowest_BIC,parent[0],cp_mat_list[0],alpha,beta,maxOWsize,
                                           min_num_obs_for_split,min_num_obs_after_split,//,first_round
                                           root_alg_precision,
                                           maxit,
  ...
  for(int k=0;k<overall_trees.size();k++){
    ...
    //COMMENTED OUT THE LINES BELOW. Should not be used for logit-BART-BMA
    // if(overall_parent2[k]==-1){
    //   //Rcout << "Line 1949. k = " << k << ". \n";
    //   new_mean=update_mean_var(overall_trees[k],overall_mat[k],
    //                            resids(_,0),a);
    // }else{
    //   //Rcout << "Line 1952. k = " << k << ". \n";
    //   //Rcout << "Line 1952. overall_parent2[k] = " << overall_parent2[k] << ". \n";
    //   new_mean=update_mean_var(overall_trees[k],overall_mat[k],
    //                            resids(_,overall_parent2[k]),a);
    // }
    // //Rcout << "Line 1953. k = " << k << ". \n";
    // //terminal_nodes=find_term_nodes(overall_trees[k]);
    // updated_preds=update_predictions(overall_trees[k],overall_mat[k],new_mean,D1.n_rows);
...
List get_best_trees_update_splits_exact(double less_greedy, double spike_tree, int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
                                        arma::mat& D1,
                                        arma::vec y_arma,NumericMatrix resids,
                                        double a,double mu,double nu,double lambda,double c,
                                        double sigma_mu,List tree_table,List tree_mat,double lowest_BIC,//int first_round,
                                        IntegerVector parent,List cp_mat_list,//IntegerVector err_list,
                                        NumericMatrix test_data,double alpha,double beta,bool is_test_data,double pen,int num_cp,bool split_rule_node,bool gridpoint,int maxOWsize,int num_splits,int gridsize, bool zero_split,
    ...
    for(int i=0;i<tree_table.size();i++){
      ...
      if(split_rule_node==1){
        if(j==0){
          ...
          best_subset=get_best_split_2_exact(less_greedy,spike_tree, s_t_hyperprior, p_s_t, a_s_t,b_s_t,num_obs,num_vars,lambda_poisson,
                                             y_arma, //resids(_,0),
                                             D1,tree_table[i],tree_mat[i],a,mu,nu,lambda,log(c),
                                             lowest_BIC,parent[0],cp_mat_list[i],alpha,beta,maxOWsize,
                                             min_num_obs_for_split,min_num_obs_after_split,//,first_round
  ...
  for(int k=0;k<overall_trees.size();k++){
    ...
    //Commented out code below because prediction calculations are different for logit-BART-BMA
    // if(overall_parent2[k]==-1){
    //   //Rcout << "Line 1949. k = " << k << ". \n";
    //   new_mean=update_mean_var(overall_trees[k],overall_mat[k],resids(_,0),a);
    // }else{
  ...
  ret[2]=overall_mat;
  ret[3]=overall_parent2;
  ret[4]=overallpreds_total;//wrap(M1);
  ret[5]=lowest_BIC;
  //ret[6]=wrap(M2);
  //ret[7]=overallpreds_total;
  return(ret);
...
List get_best_trees_sum_exact(double less_greedy,
                              double spike_tree,
                              int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
                              arma::mat& D1,NumericMatrix resids,double a,double mu,double nu,double lambda,
                              double c,double sigma_mu,List tree_table,List tree_mat,double lowest_BIC,//int first_round,
                              IntegerVector parent,List cp_mat_list,IntegerVector err_list,NumericMatrix test_data,
                              double alpha,double beta,bool is_test_data,double pen,int num_cp,bool split_rule_node,bool gridpoint,int maxOWsize,List prev_sum_trees,
                              List prev_sum_trees_mat,
                              arma::vec y_arma,//NumericVector y_scaled,
                              int num_splits,int gridsize,bool zero_split,
                              unsigned int min_num_obs_for_split, unsigned int min_num_obs_after_split,
  ...
  if(zero_split==1){
    for(int q=0; q<parent.size();q++){
      ...
      if(is<List>(s_temp)){
        ...
        List lik_listtemp=sumtree_likelihood_function2_exact(y_arma,sum_trees2_temp,sum_trees_mat2_temp,
                                                             y_arma.n_elem,a,nu,lambda,
                                                             root_alg_precision,
                                                             maxit,
                                                             eps_f,
                                                             eps_g);
        ...
        List lik_listtemp=sumtree_likelihood_function2_exact(y_arma,st,st_mat,
                                                             y_arma.n_elem,a,nu,lambda,
                                                             root_alg_precision,
                                                             maxit,
                                                             eps_f,
  ...
  for(int j=0;j<num_splits;j++){
        ...
        if(split_rule_node==1){
          if(j==0){
            best_subset=get_best_split_sum_exact(less_greedy,spike_tree, s_t_hyperprior, p_s_t, a_s_t,b_s_t,num_obs,num_vars,lambda_poisson,D1,tree_table[i],tree_mat[i],a,mu,nu,lambda,log(c),lowest_BIC,
                                                 parent[i],cp_mat_list[i],
                                                 alpha,beta,maxOWsize,//first_round,
                                                 prev_sum_trees,prev_sum_trees_mat,
                                                 y_arma, //y_scaled,
                                                 parent,i,
                                                 min_num_obs_for_split,min_num_obs_after_split,
                                                 root_alg_precision,
    ...
    if(split_rule_node==1){
      ...
      for(int k=0;k<table_subset_curr_round.size();k++){
        ...
        if(parent_curr_round[k]==-1){
          test_res=resids(_,0);
          arma::vec tempprobvec = prev_round_preds2.col(0);
          p_1_minus_p = tempprobvec%(1-tempprobvec);
        }else{
          test_res=resids(_,parent_curr_round[k]);
          arma::vec tempprobvec = prev_round_preds2.col(parent_curr_round[k]);
          p_1_minus_p = tempprobvec%(1-tempprobvec);
        ...
        List cp_mat_list1;
        if(gridpoint==0){
          cp_mat_list1=make_pelt_cpmat(wrap(D1),
                                       test_res,
                                       pen,num_cp);
        }else{
          cp_mat_list1=make_gridpoint_cpmat(wrap(D1),
                                            test_res,
                                            gridsize,num_cp,
                                            p_1_minus_p);
        }
...
List get_best_trees_sum_update_splits_exact(double less_greedy, double spike_tree, int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
                                            arma::mat& D1,
                                            NumericMatrix resids,
                                            double a,double mu,double nu,double lambda,
                                            double c,double sigma_mu,List tree_table,List tree_mat,double lowest_BIC,//int first_round,
                                            IntegerVector parent,List cp_mat_list,IntegerVector err_list,NumericMatrix test_data,double alpha,double beta,bool is_test_data,double pen,int num_cp,bool split_rule_node,bool gridpoint,int maxOWsize,List prev_sum_trees,List prev_sum_trees_mat,
                                            arma::vec y_arma, //NumericVector y_scaled,
                                            int num_splits,int gridsize,bool zero_split,
                                            unsigned int min_num_obs_for_split,
                                            unsigned int min_num_obs_after_split,
                                            arma::mat prev_round_preds2,
                                            double root_alg_precision,
                                            int maxit,
                                            double eps_f,
  ...
  if(zero_split==1){
    for(int q=0; q<parent.size();q++){
      ...
      if(is<List>(s_temp)){
        ...
        List lik_listtemp=sumtree_likelihood_function2_exact(y_arma,
                                                             sum_trees2_temp,sum_trees_mat2_temp,
                                                             y_arma.n_elem,
                                                             a,nu,lambda,
                                                             root_alg_precision,
                                                             maxit,
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List preds_logit_bbma_insamp_with_ints(List overall_sum_trees,
                                       List overall_sum_mat,
                                        arma::vec y_arma,// NumericVector y,
  ...
  arma::mat overall_map_xbeta(num_obs,num_models);
  arma::mat t_vars_arma(num_obs,num_models);
  ...
  for(int i=0;i<overall_sum_trees.size();i++){
    ...
    //overall_preds.col(j)=predprobstemp;
    overall_map_xbeta.col(i)=wo_means;
    t_vars_arma.col(i)=whw_vars;


    double weight=exp(BICi_arma[i]-(max_BIC+log(sum(arma::exp(BICi_arma-max_BIC)))));
    weights_arma(i) = weight ;
    preds_all_models_arma.col(i)=predprobstemp*weight;
 ...
 // NumericVector orig_preds=get_original(min(y),max(y),-0.5,0.5,wrap(predicted_values)) ;
  ...
  std::vector<double> weights_vec= arma::conv_to<stdvec>::from(weights_arma);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List preds_logit_bbma_insamp_no_ints(List overall_sum_trees,List overall_sum_mat,
                                       arma::vec y_arma,// NumericVector y,
                                       NumericVector BIC_weights,//int num_iter,int burnin,
                                       int num_obs,
                                       double a,//double sigma,double mu_mu,double nu,double lambda,
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List preds_logit_bbma_outsamp_with_ints(List overall_sum_trees,
                                        List overall_sum_mat,
                                        arma::vec y_arma,//NumericVector y,
                                        NumericVector BIC_weights,//int num_iter,int burnin,
                                        int num_obs,
                                        int num_test_obs,
                                        double a,//double sigma,double mu_mu,double nu,double lambda,//List resids,
                                        int ncores,
                                        double root_alg_precision,
                                        int maxit,
                                        double eps_f,
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List preds_logit_bbma_outsamp_no_ints(List overall_sum_trees,
                                        List overall_sum_mat,
                                        arma::vec y_arma,//NumericVector y,
                                        NumericVector BIC_weights,//int num_iter,int burnin,
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List L_BART_BMA_sumLikelihood(double less_greedy,double spike_tree, int s_t_hyperprior, double p_s_t, double a_s_t, double b_s_t, double num_obs, double num_vars, double lambda_poisson,
                            NumericMatrix data,NumericVector y,double start_mean,double start_sd,
                            double a,double mu,double nu,double lambda,double c,
                            double sigma_mu,double pen,int num_cp,
                            NumericMatrix test_data,int num_rounds,
                            double alpha,double beta,bool split_rule_node,
                            bool gridpoint,int maxOWsize,int num_splits,
                            int gridsize, bool zero_split,bool only_max_num_trees,
                            unsigned int min_num_obs_for_split, unsigned int min_num_obs_after_split,
                            int exact_residuals,
                            int calc_intervals,
                            int ncores,
...
// Rcout << "Line 11628 . \n";
  ...
  arma::colvec y_arma=clone(y);

  // Rcout << "Line 11664 . \n";


  //	double lik=likelihood_function(y_scaled,treetable,treemat,a,mu,nu,lambda);
  double lik=as<double>(likelihood_function2_exact(y_arma,//y_scaled,
                                                   treetable,treemat,a,mu,nu,lambda,
                                                   root_alg_precision,
                                                   maxit,
                                                   eps_f,
                                                   eps_g)[0]);
  ...
  // first transformation creating latent outcome
  // set probability equal to 0,5 for all observations
  r.insert_cols(0,(y_arma-0.5)/0.25);
  ...
  for(int j=0;j<num_rounds;j++){
    //Rcout << "Line 11733 outer loop number " << j << " . \n";
    ...
    //Rcout << "Line 11754 outer loop number " << j << " . \n";
    ...
    for(int f=0;f<resids.ncol();f++){
      if(gridpoint==0){
        ...
        if(j==0){
          ...
          //arma::vec p_1_minus_p = probvectemp % (1-probvectemp);
          arma::vec p_1_minus_p = prev_round_preds2.col(f) % (1-prev_round_preds2.col(f));
          cp_mat_list=make_gridpoint_cpmat(data,resids(_,f),gridsize,num_cp,
                                           p_1_minus_p);

          //probvectemp=(0.5)*probvectemp.ones();
    ...
    // Rcout << "Line 11804 outer loop number " << j << " . \n";
    ...
    // Rcout << "Line 11820 outer loop number " << j << " . \n";
    ...
    if(exact_residuals==1){
      ...
      if(j==0){
        if(split_rule_node==1){
          ...
          CART_BMA=get_best_trees_update_splits_exact(less_greedy,spike_tree, s_t_hyperprior, p_s_t, a_s_t,b_s_t,num_obs,num_vars,lambda_poisson,D1,
                                                      y_arma,
                                                      resids,
                                                      a,mu,nu,lambda,c,sigma_mu,tree_table,tree_mat,lowest_BIC,//first_round,
          ...
          CART_BMA=get_best_trees_exact(less_greedy,spike_tree, s_t_hyperprior, p_s_t, a_s_t,b_s_t,num_obs,num_vars,lambda_poisson,D1,
                                        y_arma,
                                        resids,
                                        a,mu,nu,lambda,c,sigma_mu,tree_table,tree_mat,lowest_BIC,//first_round,
                                        parent,resids_cp_mat,//as<IntegerVector>(wrap(err_list)),
        ...
        if(split_rule_node==1){
          CART_BMA=get_best_trees_sum_update_splits_exact(less_greedy,spike_tree, s_t_hyperprior, p_s_t, a_s_t,b_s_t,num_obs,num_vars,lambda_poisson,D1,
                                                          resids,
                                                          a,mu,nu,lambda,c,sigma_mu,tree_table,tree_mat,lowest_BIC,//first_round,
                                                          parent,resids_cp_mat,as<IntegerVector>(wrap(err_list)),test_data,alpha,beta,is_test_data,pen,num_cp,split_rule_node,gridpoint,maxOWsize,prev_sum_trees,prev_sum_trees_mat,
                                                          y_arma,//y_scaled,
          ...
          CART_BMA=get_best_trees_sum_exact(less_greedy,spike_tree, s_t_hyperprior, p_s_t, a_s_t,b_s_t,num_obs,num_vars,lambda_poisson,D1,
                                            resids,
                                            a,mu,nu,lambda,c,sigma_mu,tree_table,tree_mat,lowest_BIC,//first_round,
                                            parent,resids_cp_mat,as<IntegerVector>(wrap(err_list)),test_data,alpha,beta,is_test_data,pen,num_cp,split_rule_node,gridpoint,maxOWsize,prev_sum_trees,prev_sum_trees_mat,
                                            y_arma,//y_scaled,
      ...
      Rcout << "Currently does not support exact_residuals==0. \n";
    //   if(j==0){
    //     if(split_rule_node==1){
    ...
    // Rcout << "Line 11934 outer loop number " << j << " . \n";
    ...
    //commented out for logit-BART-BMA
    ...
    if(exact_residuals==1){
      ...
      NumericMatrix temp_truepredmat = CART_BMA[4];
      curr_true_preds=temp_truepredmat;
    }
    arma::mat curr_predmat_arma = as<arma::mat>(curr_true_preds);
    // Rcout << "Line 9640 outer loop number " << j << " . \n";

    //
    //Rcout << "curr_round_preds = " << curr_round_preds << ".\n";
    // Rcout << "Line 11973 outer loop number " << j << " . \n";
    ...
    // Rcout << "Line 12013 outer loop number " << j << " . \n";
    ...
    for(int k=0;k<curr_round_lik.size();k++){
      ...
      if(j==0){
        ...
        if(exact_residuals==1){
          ...
          // Rcout << "exact_residuals==0 not supported. Line 10580. j=  " << j << " . \n";
          //temp_preds(_,k)=curr_round_preds(_,k);
        }
        //if(is_test_data==1) temp_test_preds(_,k)=curr_round_test_preds(_,k);

        //might require trimming of probabilities here
        //temp_resids(_,k)=(y-temp_preds(_,k))/(temp_preds(_,k)%(1-temp_preds(_,k)));

        //IDEALLY EVERYTHING WOULD BE IN ARMADILLO OBJECTS
        NumericVector temp_trans_y = wrap((y_arma-curr_predmat_arma.col(k))/(curr_predmat_arma.col(k)%(1-curr_predmat_arma.col(k))));
        temp_resids(_,k)= temp_trans_y;
        ...
        if(exact_residuals==1){
          ...
          //temp_resids(_,k)=wrap((y_arma-curr_predmat_arma.col(k))/(curr_predmat_arma.col(k)%(1-curr_predmat_arma.col(k))));

          NumericVector temp_trans_y = wrap((y_arma-curr_predmat_arma.col(k))/(curr_predmat_arma.col(k)%(1-curr_predmat_arma.col(k))));
          temp_resids(_,k)= temp_trans_y;

        }else{
          Rcout << "Exact resids not supported. Line 10610. j=  " << j << " . \n";
    ...
    // Rcout << "Line 12087 outer loop number " << j << " . \n";
    ...
    // Rcout << "Line 12186 outer loop number " << j << " . \n";
  ...
  if(overall_lik.size()==0){
    ...
    // NEED TO REWRITE CODE OBTAINING PREDICTIONS
    // (in-sample and out of sample)
    // and obtaining intervals etc.
    // for final set of chosen models
    // Rcout << "Line 12400 . \n";

    List final_preds_insamp;
    List final_preds_outsamp;
    // If want credible intervals to be calculated
    if(calc_intervals == 1){
      // Rcout << "Line 12406 . \n";

      final_preds_insamp = preds_logit_bbma_insamp_with_ints(overall_overall_sum_trees,
                                                                  overall_overall_sum_trees_mat,
                                        y_arma, //NumericVector y,
                                        end_BIC,//int num_iter, int burnin,
                                        num_obs,
                                        a,//double sigma,double mu_mu,double nu,double lambda,
                                        ncores,
                                        root_alg_precision,
                                        maxit,
                                        eps_f,
      ...
      if(is_test_data==1){
        ...
        final_preds_outsamp = preds_logit_bbma_outsamp_with_ints(overall_overall_sum_trees,
                                                                      overall_overall_sum_trees_mat,
                                           y_arma,//NumericVector y,
                                           end_BIC,//int num_iter, int burnin,
                                           num_obs,
                                           test_data.nrow(),
                                           a,//double sigma,double mu_mu,double nu,double lambda,//List resids,
                                           ncores,
                                           root_alg_precision,
                                           maxit,
                                           eps_f,
      ...
      }//close if statement (out of sampke predictions)


    }else{ // If don't want credible intervals to be calculated

      // Rcout << "Line 12447 . \n";

      final_preds_insamp = preds_logit_bbma_insamp_no_ints(overall_overall_sum_trees,
                                                                overall_overall_sum_trees_mat,
                                      y_arma,// NumericVector y,
                                      end_BIC,//int num_iter, int burnin,
                                      num_obs,
      ...
      if(is_test_data==1){
        ...
        final_preds_outsamp = preds_logit_bbma_outsamp_no_ints(overall_overall_sum_trees,
                                                                    overall_overall_sum_trees_mat,
                                         y_arma,//NumericVector y,
                                         end_BIC,//int num_iter, int burnin,
                                         num_obs,
    ...
    }//close else staement (credible intervals)
    ...
    if(calc_intervals == 1){
      ...
      if(is_test_data==1){
        ...
        List ret(7);
        ret[0] = final_preds_insamp[0]; //insample probabilities
        ret[1] = overall_overall_sum_trees;
        ret[2] = overall_overall_sum_trees_mat;
        ret[3] = final_preds_insamp[2]; //model weights (should sum to 1)
        ret[4] = final_preds_outsamp[0]; //out of sample probabilities
        ret[5] = final_preds_insamp[1]; //in sample credible intervals
        ret[6] = final_preds_outsamp[1]; //out of sample credible intervals
        return(ret);
      }else{
        List ret(5);
        ret[0] = final_preds_insamp[0]; //insample probabilities
        ...
        ret[2] = overall_overall_sum_trees_mat;
        ret[3] = final_preds_insamp[2]; //model weights (should sum to 1)
        ret[4] = final_preds_insamp[1]; //in sample credible intervals
        return(ret);
      }
    }else{
      if(is_test_data==1){
        ...
        ret[1] = overall_overall_sum_trees;
        ret[2] = overall_overall_sum_trees_mat;
        ret[3] = final_preds_insamp[1]; //model weights (should sum to 1)
        ret[4] = final_preds_outsamp[0]; //out of sample probabilities
        return(ret);
      }else{
        List ret(4);
        ...
        ret[3] = final_preds_insamp[1]; //model weights (should sum to 1)
        return(ret);
      }//close else statement for is_test_data==1
    }//close else statement for calc_intervals == 1
  }// close else statement for overall_lik.size()==0
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List pred_ints_lbbma_ITE_outsamp_par(List overall_sum_trees,
                               List overall_sum_mat,
                               NumericVector y,
                               NumericVector BIC_weights,//double min_possible,double max_possible,
                               int num_obs,int num_test_obs,
  ...
  int num_models = BIC_weights.size();
  int numobstemp0 = num_test_obs;


  arma::vec overall_liks(num_models);

  arma::mat overall_preds(numobstemp0,num_models);
  arma::mat weighted_preds_all_models_arma(numobstemp0,BIC_weights.size());
...
#pragma omp for
  for(int i=0;i<overall_sum_trees.size();i++){
    ...
    const Eigen::Map<Eigen::MatrixXd> xx(W.memptr(),
                                         W.n_rows,
                                         W.n_cols);
    ...
    if(is_test_data==1){
      ...
      overall_preds.col(i)=pred_ITE_temp;
      weighted_preds_all_models_arma.col(i)= pred_ITE_temp*post_weights_arma(i);
      ...
      t_vars_mats_f(i) = tempvarcube;

      if(include_cate_intervals==1){
        //Rcout << "Line 22437. \n";

        varmats_cate_f(i) = invHmat;
        Wcontr_cate_f(i) = W_tilde0;
        Wtreat_cate_f(i) = W_tilde1;
        mapnodemeans_f(i) = mapcoeffs;
      ...
      //
      //   varmats_cate_f(i) = invHmat;
      //   Wcontr_cate_f(i) = Wmat0;
      //   Wtreat_cate_f(i) = Wmat1;
      //   mapnodemeans_f(i) = mapcoeffs;
      //
      // }
...
#pragma omp barrier
  ...
  arma::imat num_samp_mat(post_weights_arma.n_elem, numobstemp0);
  for(int j=0;j<numobstemp0;j++){
    IntegerVector num_its_to_sample = RcppArmadillo::rmultinom(num_iter,
                                                               post_weights);
  ...
  if(include_cate_intervals==1){
    ...
    ret(2) = arma::mean(predicted_values);
...
// [[Rcpp::depends(RcppArmadillo)]]
// [[Rcpp::export]]
List pred_ints_lbbma_ITE_insamp_par(List overall_sum_trees,
                              List overall_sum_mat,
                              NumericVector y,
                              NumericVector BIC_weights,//double min_possible,double max_possible,
                              int num_obs,//int num_test_obs,
...
#pragma omp for
  for(int i=0;i<overall_sum_trees.size();i++){
    ...
    //if(is_test_data==1){
      //W_tilde is the insample data here
